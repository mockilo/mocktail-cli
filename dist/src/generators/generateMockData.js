...
const baseGenerators_1 = require("./baseGenerators");
...
function generateMockData(model, options = {}) {
    const { count = 10, customFields = {}, relationData: initialRelationData = {}, config = {}, preset = null } = options;
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (const field of model.fields) {
            ...
            if (customFields && customFields[field.name]) {
                let val = typeof customFields[field.name] === 'function'
            ...
            // ID generation
            if (field.isId) {
                const idVal = (0, baseGenerators_1.generateField)(field); // Use proper field generation for IDs
                rec[field.name] = sqlMode ? safeValue(idVal, { sqlMode }) : idVal;
                continue;
            }
            // Generate scalars
            if (field.isScalar) {
                const v = (0, baseGenerators_1.generateField)(field); // Pass the whole field object
        ...
        for (const rec of records) {
            for (const field of model.fields) {
                ...
                if (field.isArray) {
                    ...
                    const presetCount = relationData[relName]?.count;
                    let n;
                    if (presetCount) {
...
function generateCustomRelations(model, preset, relationData) {
    ...
    for (const [relationName, config] of Object.entries(modelConfig || {})) {
        if (config.count) {
            ...
            const count = faker_1.faker.number.int(config.count);
