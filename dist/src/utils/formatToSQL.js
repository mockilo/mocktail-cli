...
exports.formatToSQL = formatToSQL;
exports.formatJoinTableSQL = formatJoinTableSQL;
...
function formatToSQL(modelName, records, fieldsMeta = []) {
    ...
    const rows = records
        .map(record => `(${allowedColumns.map(col => escapeSqlValue(record[col])).join(', ')})`)
        .join(',\n');
...
function formatJoinTableSQL(joinTableName, records) {
    ...
    const rows = records
        .map(record => `(${columns.map(col => escapeSqlValue(record[col])).join(', ')})`)
        .join(',\n');
